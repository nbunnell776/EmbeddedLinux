...
// Typdef an enum to use as a thread status flag
typedef enum {FREE, IN_USE, PENDING} used;

// Create struct to hold thread meta data
typedef struct {
	used flag;			// thread status flag
	int socket;			// socket number
	pthread_t thread;	// pointer to thread
} meta_pthread_t;

// Allocate array of meta_pthreads, arbitrary limit as opposed to dynamic malloc
#define THREADCOUNT 5
meta_pthread_t mPThreadArray[THREADCOUNT];
    ...
    while (1)
    {
        ...
        while (cmd)
        {
            ...
            switch (*cmd)
            {
                ...
                // Add case p to read current param values
                case 'p':
                	snprintf(buffer, sizeof(buffer), "SP is: %d\nLimit is: %d\nDB is: %d\n", setpoint, limit, deadband);
					write(socketNumber, buffer, strlen(buffer));
					break;

				// Add case q for client termination
                case 'q':
                	// Set meta_pthread_t instance used flag to pending
...
*/
    ...
    error = pthread_create (&serverT, NULL, createServer, NULL);
    CHECK_ERROR;
...
*/
{
    ...
    pthread_join (serverT, &thread_val);
}

// Recast as thread type w/ void* arg and return
void *createServer(void *arg)
{
...
// Accept a connection
    while (1)
    {
		...
		// Replace the return client_socket; statement with a call to pthread_create() to create a monitor thread
		// return (client_socket);
		pthread_create (&monitorT, NULL, monitor, NULL);
    }
